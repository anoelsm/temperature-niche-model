...
kval <- 3
  for (e in c(1, 3)){
    ...
    for (en in 1:100){
      ####### Initial Values ----
      start_vals <- c(0, 1E-4, 1E-3, 1E-2, 1E-1, 1E0, 1E1)
      for (i in 1:bx){
        P[1,,i] <- sample(start_vals,45,replace=TRUE,prob=dpois(1:7, lambda=1))
      }
      ##Create the filename within this loop
      subtitle <- paste0("-review_ensemble_", en)
      output_filename <- paste0("T", variable, "_D", interaction, "_E", kval, "_exp", wm, subtitle)
      nc_fname <- paste(nc_path, output_filename, ".nc", sep="")
      nc_out <- nc_create(
        nc_fname,
